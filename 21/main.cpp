...
int main (int argc, char **argv) {
    ...
    float targetFrameRate = 1000.0f / 1.0f;
    ...
    while (programRunning) {
        ...
        switch (result) {
            case REQUEST_INPUT: {
                pushStringIntoInput(&comp->input, "NOT A J");
                pushStringIntoInput(&comp->input, "NOT B T");
                pushStringIntoInput(&comp->input, "OR T J");
                pushStringIntoInput(&comp->input, "NOT C T");
                pushStringIntoInput(&comp->input, "OR T J");
                pushStringIntoInput(&comp->input, "AND D J");
                pushStringIntoInput(&comp->input, "NOT E T");
                pushStringIntoInput(&comp->input, "NOT T T");
                pushStringIntoInput(&comp->input, "OR H T");
                pushStringIntoInput(&comp->input, "AND T J");
                pushStringIntoInput(&comp->input, "RUN");
            } break;
            case REQUEST_OUTPUT: {
                int64 output = listSplice(&comp->output, 0);
                if (output > 127) {
                    damageAmt = (int)output;
